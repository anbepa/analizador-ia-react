...
function BugReport({ data, flowA = [], flowB = [], onClose }) {
    ...
    const bugCount = Array.isArray(data) ? data.length : 0;
    const totalSteps = Array.isArray(data)
        ? data.reduce((sum, b) => sum + (b.pasos_para_reproducir?.length || 0), 0)
        : 0;
        ...
        <div className="mt-6" ref={reportRef}>
            ...
            {bugCount > 0 && (
                <p className="text-sm text-gray-600 mb-4">
                    Se encontraron {bugCount} bug{bugCount !== 1 && 's'}. Total de pasos analizados: {totalSteps}.
                </p>
            )}
